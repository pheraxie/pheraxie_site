...
export default function ProjetPage() {
  ...
  useEffect(() => {
    setProjects(JSON.parse(localStorage.getItem('pheraxie_projects') || '[]'));
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={styles.main} style={{ paddingTop: navHeight }}>
        <div style={{ maxWidth: 700, margin: '2rem auto', background: '#f9f9f9', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px #0001' }}>
          <h2 style={{ textAlign: 'center', marginBottom: 24 }}>Mes sites déployés</h2>
          {projects.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#888' }}>Aucun site déployé pour l'instant.</div>
          ) : (
            <ul style={{ listStyle: 'none', padding: 0 }}>
              {projects.map((proj, idx) => (
                <li key={idx} style={{ background: '#fff', borderRadius: 8, marginBottom: 16, padding: 16, boxShadow: '0 1px 4px #0001', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ fontWeight: 600, fontSize: 18 }}>{proj.siteName || 'Sans nom'}</div>
                  <div style={{ fontSize: 13, color: '#888' }}>{new Date(proj.date).toLocaleString()}</div>
                  <div style={{ marginTop: 8 }}>
                    <a href={`https://${proj.url}`} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', fontWeight: 600 }}>
                      Voir le site
                    </a>
                    <div style={{ fontSize: 13, color: '#888', marginTop: 4, wordBreak: 'break-all' }}>
                      URL : https://{proj.url}
                    </div>
                  </div>
                </li>
